...
def train(args):
    ...
    for i in range(num_epochs):
        ...
        for idx, (ims, targets, _) in enumerate(tqdm(train_dataset)):
            ...
            # Check for NaN in individual loss components before combining
            if torch.isnan(batch_losses['classification']) or torch.isnan(batch_losses['bbox_regression']):
                print(f'NaN detected in loss components: cls={batch_losses["classification"].item():.6f}, bbox={batch_losses["bbox_regression"].item():.6f}')
            ...
            if steps % train_config['log_steps'] == 0:
                loss_output = ''
                loss_output += 'SSD Classification Loss : {:.4f}'.format(np.mean(ssd_classification_losses) if ssd_classification_losses else 0.0)
                loss_output += ' | SSD Localization Loss : {:.4f}'.format(np.mean(ssd_localization_losses) if ssd_localization_losses else 0.0)
                print(loss_output)
