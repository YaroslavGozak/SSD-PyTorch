...
def infer_sequentially(args):
    ...
    model, dataset, test_dataset_loader, config = load_model_and_dataset(device=device, args=args)
    ...
    with torch.no_grad():
        while True:
            ...
            if (not paused) or step_once:
                ...
                _, detections = model(im_tensor, [target] if hasattr(model, 'training') and model.training else None)
