...
def _build_default_boxes(feature_shapes, aspect_ratios, scales):
    ...
    default_boxes = []
    for feat_idx, (feat_h, feat_w) in enumerate(feature_shapes):
    ...
    return default_boxes.clamp(min=0.0, max=1.0)
...
def generate_default_boxes(features, aspect_ratios, scales, image_size):
    ...
    feature_shapes = tuple((int(f.shape[-2]), int(f.shape[-1])) for f in features)
    cache_key = (
        feature_shapes,
    ...
    return [dboxes_in_image] * features[0].size(0)
