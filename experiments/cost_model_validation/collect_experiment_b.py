...
from .artifacts import load_calibration, reference
from .shape_model import MergePolicy, policy_declaration
...
def collect(config, fit_path: str, output: str, tau_override: float | None = None, overwrite: bool = False, pairs_path: str | None = None, allow_reused_pairs: bool = False):
    import json
    output_dir = Path(output)
    output_dir.mkdir(parents=True, exist_ok=True)
    raw_path = output_dir / "experiment_b_raw.csv"
    fit = load_calibration(fit_path)
    ...
    quality_warnings.extend(check_freshness(specs_path,specs,config,allow_reused_pairs))
    policy = MergePolicy(fit,policy_declaration(config))
    strict_lookup = experiment.get("strict_lookup",False) or config.get("publication_run",False)
    if experiment.get("confirmatory") and fit.get("policy_declaration") != policy_declaration(config):
        raise ValueError("Calibration policy declaration differs from requested validation")
    candidate_predictions = {}
    for index,pair in enumerate(pairs):
        shapes = [p.tensor_hw for p in pair["prepared"]]
        values = {}
        for name in ("shape_lookup","shape_lookup_conservative","conservative_consensus"):
            if name.startswith("shape_lookup") and not strict_lookup and (policy.lookup is None or not all(policy.lookup.is_in_domain(*hw) for hw in shapes)):
                continue
            values[name] = policy.predict(shapes,name,strict=True)
        candidate_predictions[str(pair["specification"]["pair_id"])] = values
    ...
    if raw_path.exists():
        ...
        raw_path.unlink()
    repetitions_by_pair = {i:repetitions_for_pair(config,p["specification"]) for i,p in enumerate(pairs)}
    randomizer = random.Random(seed)
    session_id = str(uuid.uuid4())
    session_start = __import__("time").perf_counter()
    orders = {pair_id: balanced_orders(repetitions_by_pair[pair_id], seed + pair_id) for pair_id in range(len(pairs))}
    blocks = [(pair_id, list(range(start,min(start+4,repetitions_by_pair[pair_id])))) for pair_id in range(len(pairs)) for start in range(0,repetitions_by_pair[pair_id],4)]
    ...
    control_records = []
    # Freeze the declaration and predictions before any measured outcomes exist.
    write_json(output_dir / "validation_declaration.json",specs["payload"].get("validation_declaration",{}))
    write_json(output_dir / "pair_specs.json",specs)
    write_json(output_dir / "candidate_predictions.json",candidate_predictions)
    import shutil
    calibration_copy = output_dir / "calibration"
    calibration_copy.mkdir(exist_ok=True)
    shutil.copy2(fit_path,calibration_copy / "linear_fit.json")
    for key in ("shape_statistics_reference","control_records_reference"):
        if fit.get(key):
            source = Path(fit_path).parent / fit[key]["path"]
            shutil.copy2(source,calibration_copy / source.name)
    if experiment.get("confirmatory",False):
        mark_used(specs_path,specs["sha256"],output_dir)
    for completed_trial, (pair_id, repetition) in enumerate(trials, 1):
        repetitions = repetitions_by_pair[pair_id]
        pair = pairs[pair_id]
    ...
    diagnostics = control_diagnostics(fit,control_records,experiment)
    hardware_shift = diagnostics["hardware_state_shift"]
    drift_error = None
    try:
        gate(not hardware_shift, "calibration_to_validation_shift", config, quality_warnings)
        gate(not diagnostics["within_session_drift_detected"], "within_session_drift", config, quality_warnings)
    ...
    write_json(output_dir / "experiment_b_metadata.json", {**system_metadata(), "config": config, "quality_warnings": quality_warnings, "tau_used": tau, "tau_override": tau_override is not None, "session_id": session_id,
                                                            "pair_count": len(pairs), "boundary_pair_count": sum(any(tag.endswith("_boundary") for tag in pair["specification"]["stratum_tags"]) for pair in pairs),
                                                            ...
                                                            "schema_version": 4, "pair_specs_hash": specs["sha256"], "pair_specs_reference": reference(output_dir / "pair_specs.json"),
                                                            "candidate_predictions_reference": reference(output_dir / "candidate_predictions.json"),
                                                            "raw_observations_reference": reference(raw_path), "provenance": current,
                                                            "evaluation_design": specs["payload"].get("evaluation_design","challenge"),
                                                            "aggregate_scope": specs["payload"].get("aggregate_scope","unweighted_challenge_average"),
                                                            "policy_declaration": policy.declaration,"fallback_counts":dict(policy.fallback_counts),
                                                            "control_diagnostics": diagnostics,
                                                            "confirmatory": experiment.get("confirmatory",False) and not any("NON-CONFIRMATORY" in w for w in quality_warnings),
                                                            "calibration_reference": {"path": "calibration/linear_fit.json", "content_hash": file_sha256(fit_path)},
                                                            "calibration_provenance": fit.get("provenance", {}),
                                                            "sampling_basis": specs["payload"].get("reference_distribution") or "frozen_multi_model_strata", "sampling_tau_pixels": tau, "boundary_quotas": quota_fractions,
...
def main():
    ...
    parser.add_argument("--allow-reused-pairs",action="store_true",help="Explicit non-confirmatory reuse; records a warning")
    args = parser.parse_args(); collect(load_config(args.config), args.fit, args.output, args.tau_override, args.overwrite, args.pairs,args.allow_reused_pairs)
